...
import * as engines from "./mod.ts";

Deno.test("PlantUML GraphViz DOT ER Diagram Transformer", async () => {
  await testRdbmsModelTransform(
    engines.PlantUmlGraphvizTransformer,
    trSqlDia.SQLiteDialect,
    "mod_test.graphviz-dot.puml.golden",
  );
});

Deno.test("PlantUML Information Engineering Diagram Transformer", async () => {
  await testRdbmsModelTransform(
    engines.PlantUmlInfoEngrModelTransformer,
    trSqlDia.PostgreSqlCommonPkColNamedIdDialect,
    "mod_test.ie-model.puml.golden",
  );
});
...
export const testSuiteGoldenFilePaths = [
  ...
  "./diagram",
  "./rds/diagram",
  "./governed-im-transform/rds/diagram",
];
